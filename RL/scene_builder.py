...
class SceneBuilder:
    ...
    def __init__(
        ...
        file_path = "franka_emika_panda/dual_panda_scene.xml",
    ):
    ...
    def build_dual_arm_robot_scene(self):
        ...
        # cubes = [[-0.08245498991209693, -0.45923869801302486, 0.21999999999999997], [0.20065155039027238, -0.459384362442892, 0.21999999999999997]]
        ...
        cubes = [[-0.32106674642726507, -0.47760148291640025, 0.21999999999999997], [0.1288107660195263, -0.3808533211756263, 0.21999999999999997]]
        trays = [[0.27120920325891248, -0.5219683622097229, 0.205], [-0.0928473876791149, -0.5247787422670399, 0.205]]
        ...
        data.qpos[:] = self.set_initial_qpos(model, cubes)
        mujoco.mj_forward(model, data)
        
        return model, data, [cubes[0], cubes[1], trays[0], trays[1]]

    def set_initial_qpos(self, model, cubes, arm_home=None):
        """
        Initialize qpos for a dual-arm robot and two free-joint cubes.

        Args:
            model: MuJoCo model (used for nq)
            data: MuJoCo data (qpos will be written to)
            cubes: list/tuple of length 2, each an (x, y, z) position
                [red_cube_pos, green_cube_pos]
            arm_home: optional list/array of 9 joint positions for one arm
        """
        if arm_home is None:
        ...
        # Two arms (9 DoF each)
        initial_qpos[0:9] = arm_home
        initial_qpos[9:18] = arm_home

        # Cube free joints: [x, y, z, qw, qx, qy, qz]
        # Red cube: indices 18–24
        initial_qpos[18:21] = cubes[0]
        initial_qpos[21:25] = [1, 0, 0, 0]

        # Green cube: indices 25–31
        initial_qpos[25:28] = cubes[1]
        initial_qpos[28:32] = [1, 0, 0, 0]
